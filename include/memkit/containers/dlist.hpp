...
enum class dlist_policy : unsigned {
    none         = 0u,
    fixed_pool   = 1u << 0u,
    heap_dynamic = 1u << 1u,
};
...
public:
    ...
    ~DList() { clear(); release_pool(); }

    [[nodiscard]] static constexpr std::size_t node_stride() noexcept
    {
        return detail::dlist_core<detail::typed_element_policy<T>>::node_stride(sizeof(T));
    }

    [[nodiscard]] static constexpr std::size_t pool_bytes(std::size_t node_capacity) noexcept
    {
        return detail::dlist_core<detail::typed_element_policy<T>>::pool_bytes(
            node_capacity,
            sizeof(T)
        );
    }
    ...
    [[nodiscard]] status init(std::byte* node_pool, std::size_t node_capacity) noexcept
    {
        ...
        return core_.init(
            ...
            node_pool,
            node_capacity,
            detail::dlist_policy::fixed_pool
        );
    }
    ...
    [[nodiscard]] T* back() noexcept { return static_cast<T*>(core_.back()); }
    [[nodiscard]] const T* back() const noexcept { return static_cast<const T*>(core_.back()); }
    ...
    {
        return core_.for_each_reverse([&visit](const void* elem, std::size_t index) {
            if constexpr (std::is_same_v<
                              std::invoke_result_t<Visitor, const T&, std::size_t>,
                              status>) {
                return visit(*static_cast<const T*>(elem), index);
...
private:
    ...
    detail::dlist_core<detail::typed_element_policy<T>> core_{};
